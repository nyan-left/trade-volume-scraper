...
let unique = arr.filter((v,i,a)=>a.findIndex(t=>(t.name === v.name))===i)

type t = {
  id : number,
  name : string
}
let enums : string = 'enum items {';

for (let item of (unique as t[])){
  enums += `"${item.name.replace(' ', '_')}" = ${item.id},\n`
}
// todo - 
// node out/enumGenerator >> src/enums.ts
